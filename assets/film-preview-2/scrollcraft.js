/* ============================================================================
   ...
   ---------------------------------------------------------------------------
     data-sc-scrub            on <video>. p scrubs currentTime. Native media loading
                              streams only the ranges requested by the browser.
...
(function (global) {
  ...
  function mount(root, opts) {
    ...
    function makeClip(v, host) {
      var rec = {
        el: v, host: host || v,
        ready: false, loading: false, painted: false, failed: false,
    ...
    function loadClip(V) {
      // The viewer is opened explicitly. Reduced motion and Data Saver still
      // retain the poster story; neither preference needs video downloads.
      if (reduce || (navigator.connection && navigator.connection.saveData) ||
          !V || V.loading || V.failed) return;
      var src = (isMobile() && V.el.getAttribute('data-sc-src-mobile')) ||
                V.el.getAttribute('data-sc-src');
      if (!src) return;
      V.loading = true;

      function reveal() {
        if (V.painted || V.failed || V.el.readyState < 2 ||
            !V.el.videoWidth || V.el.seeking) return;
        V.painted = true;
        V.host.classList.add('sc-has-clip');
        V.el.classList.add('sc-has-clip');
      }
      function revealDecodedFrame() {
        if (V.painted || V.failed) return;
        if ('requestVideoFrameCallback' in V.el) {
          if (V.framePending) return;
          V.framePending = true;
          V.el.requestVideoFrameCallback(function () {
            V.framePending = false;
            reveal();
          });
        } else {
          // loadeddata/seeked plus HAVE_CURRENT_DATA guarantees current frame
          // data exists. Keep the poster through a paint before revealing it.
      ...
      V.el.addEventListener('loadedmetadata', function () {
        V.ready = true;
        V.cur = V.target;
        try { V.el.currentTime = Math.max(V.target * V.el.duration, 0.001); } catch (e) {}
        read();
        primeClip(V);
      }, { once: true });
      V.el.addEventListener('loadeddata', revealDecodedFrame);
      V.el.addEventListener('seeked', revealDecodedFrame);
      V.el.addEventListener('error', function () {
        // A failed clip remains a poster. Re-reading scroll must never trigger
        // an unbounded retry loop or expose a blank video surface.
        V.failed = true;
        V.ready = false;
        V.painted = false;
    ...
    function readWorld(W) {
      ...
      for (i = 0; i < W.segs.length; i++) {
        ...
        if (t > s.c0 - 0.25 && t < s.c1 + 0.25) {
          if (s.poster && s.poster.hasAttribute('data-sc-poster-src')) {
            s.poster.src = s.poster.getAttribute('data-sc-poster-src');
            s.poster.removeAttribute('data-sc-poster-src');
          }
          if (s.clip) loadClip(s.clip);
        }
      ...
      for (var c = 0; c < W.copies.length; c++) {
        ...
        var on = vis > 0.5;
        if (on !== (q.state === 1) || q.state === -1) {
          q.state = on ? 1 : 0;
          q.el.style.pointerEvents = on ? 'auto' : 'none';
          q.el.inert = !on;
          q.el.setAttribute('aria-hidden', String(!on));
        }
      }
    ...
    function tick() {
      ...
      for (var i = 0; i < playheads.length; i++) {
        var V = playheads[i];
        if (!V.ready) continue;
        // A native range request can take time on a slow connection. Wait for
        // its seek to finish instead of repeatedly replacing the request.
        if (V.el.seeking || !V.live) continue;
        V.stuckAt = 0;
    ...
    function primeClip(V) {
      ...
      if (V.failed || V.primed || V.priming || !V.el.src) return;
      V.priming = true;
      var done = function () {
        V.priming = false;
